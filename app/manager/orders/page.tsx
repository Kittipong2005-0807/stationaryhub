  ...
  const fetchRequisitionDetails = async (requisitionId: number) => {
    try {
      ...
      const response = await fetch(
        getApiUrl(`/api/requisitions/${requisitionId}`)
      );
      const data = await response.json();
      ...
      if (response.ok) {
        ...
        console.log(
          '🖼️ Items with photos:',
          data.REQUISITION_ITEMS?.map((item: any) => ({
          ...
          <Grid item xs={12} sm={4}>
            <Card className="glass-card text-center">
              <CardContent>
                <Typography
                  variant="h4"
                  className="font-bold text-yellow-600 mb-2"
                >
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <div className="text-gray-600 text-sm">รายละเอียดคำขอเบิก</div>
          </div>
        </DialogTitle>
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <Badge
                          className={`${getStatusColor(selectedRequisition.STATUS)} border px-2 py-1 rounded-full text-xs`}
                        >
